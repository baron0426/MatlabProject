...
if(x_left<8)
    hall_color((pic_size(1)+1):(pic_size(1)+x_left),:,:) = ...
        repmat(hall_color(pic_size(1),:,:),x_left,1,1);
end
if(y_left<8)
    hall_color(:,(pic_size(2)+1):(pic_size(2)+y_left),:) = ...
        repmat(hall_color(:,pic_size(2),:),1, y_left,1);
end
...
%cover the 'chessboard' on the original picture
...
image(hall_proc);
imwrite(hall_proc, 'hall_withChessboard.jpg');