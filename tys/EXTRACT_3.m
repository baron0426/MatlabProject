% 3.2.2.£®2£©
function M = EXTRACT_3()
load('hall.mat');
load('JpegCoeff.mat');
load('hide_3_half.mat');
...
hall_color_2 = uint8(zeros(P,W));
x = 1;
y = 1;
for i = 1:P*W/64
    c0 = unzigzag(hall_color_1(:,i));
    hall_color_2(x:x+7,y:y+7) = uint8(c0);
    y = y+8;
    if y > W
        y = 1;
        x = x+8;
...
hall_gray_quantum = hall_gray;
for i = 1:8:P-7
    for j = 1:8:W-7
        part = hall_gray(i:i+7,j:j+7);
        c = dct2(part-128);
        hall_gray_quantum(i:i+7,j:j+7) = round(c./QTAB);
    end
end

hall_color_3 = double(hall_color_2) - double(hall_gray_quantum);
a = zeros(7,floor(P*W/7));
for i = 1:2:P*W
    a((i+1)/2) = hall_color_3(i);
end
a = a';
L = length(a(:,1));
M = [];
...
y = 1;
for i = 1:P*W/64
    c = double(hall_color_2(x:x+7,y:y+7)).*QTAB;
    part1 = idct2(c)+128;
